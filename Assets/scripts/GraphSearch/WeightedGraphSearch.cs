...
public class WeightedGraphSearch {
	 ...
	 * was: HashSet< Vertex >
	 */
	private HashSet< VertexOld > closedVertices;
	
	/**
	 * set of vertices we still need to visit in the search
	 */
	private HashSet< VertexOld > openVertices;
	
	/**
	 * linked list holding the path from the source vertex to the target vertex 
	 */
	private LinkedList< VertexOld > path ;

	/**
	 * default constructor
	 */
	public WeightedGraphSearch() {
		closedVertices = new HashSet< VertexOld >();
		openVertices = new HashSet< VertexOld >();
		weightToTarget = new Hashtable();
		predecessor = new Hashtable();
		path = new LinkedList<VertexOld>();
	}
	 ...
	 */
	public LinkedList< VertexOld > searchGraph(VertexOld source, VertexOld dest) {
		...
		while( openVertices.Count != 0 ) {
			...
			VertexOld v = getMinWeighingVertex(openVertices);
	 ...
	 * @return list of vertices if a path exists. empty list if no path was found.
	 */
	private LinkedList<VertexOld> buildPath(VertexOld dest) {
		...
		while(predecessor[next] != null ) {
			next = (VertexOld)predecessor[next];
			path.AddLast(next);
		}
	 ...
	 */
	private void findMinimalDistance(VertexOld v) {
		ICollection neighbors = v.getNeighbors();
		// check all neighbors of this vertex.
		foreach ( VertexOld neighbor in neighbors ) {
	 ...
	 */
	private int getEdgeWeight(VertexOld v1, VertexOld v2) {
	 ...
	 */
	private VertexOld getMinWeighingVertex(HashSet<VertexOld> vertices) {
		VertexOld minWeightVertex = null;
		foreach ( VertexOld v in vertices ) {
	 ...
	 */
	private int getWeightofPathtoVertex(VertexOld v) {
